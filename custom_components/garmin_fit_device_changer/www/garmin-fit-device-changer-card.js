const CARD_VERSION = "0.1.0-m1.14";

const TEXT = {
  sv: {
    title: "Garmin FIT Device Changer", profiles: "Enhetsprofiler", target: "Målenhet",
    noProfiles: "Ingen profil ännu. Importera en referens-FIT eller välj en Garmin-enhet ur SDK-katalogen.",
    product: "Product ID", serial: "Serial", software: "Programvara", profileType: "Profiltyp",
    full: "Full identitet", legacyBasic: "Föråldrad grundprofil", makeDefault: "Gör till standard", remove: "Ta bort",
    addTitle: "Lägg till enhet", addHelp: "Välj referens-FIT eller en enhet ur Garmin FIT SDK-katalogen.", refTab: "Referens-FIT", recommended: "Rekommenderad", manualTab: "SDK-katalog",
    ...
    manualTitle: "Välj enhet ur SDK-katalogen", manualHelp: "Välj Garmin-modell ur den lokala katalogen som genererats från Garmin FIT SDK.",
    model: "Garmin-modell",
    fullHelp: "Profilen använder full Garmin-identitet. Serienummer och firmware måste höra till just den valda Garmin-enheten.",
    legacyHelp: "Den här förhandsversionsprofilen saknar full Garmin-identitet och kan inte längre användas för patchning. Ta bort den och skapa enheten på nytt.",
    serialNumber: "Serienummer", firmware: "Firmwareversion", saveManual: "Spara enhetsprofil", catalog: "Garmin-katalog",
    patchTitle: "Patcha FIT-fil", sourceFile: "FIT-fil att patcha", targetCheck: "Målenhet i patchad FIT", patch: "Patcha FIT", download: "Hämta patchad FIT",
    changed: "Ändrade creator-fält", verified: "Övrig FIT-data verifierad oförändrad.", ready: "Klar",
    importing: "Importerar referens-FIT…", imported: "Enhetsprofil importerad.", saving: "Sparar enhetsprofil…", saved: "Enhetsprofil sparad.",
  ...
  en: {
    ...
    noProfiles: "No profile yet. Import a reference FIT or choose a Garmin device from the SDK catalog.",
    product: "Product ID", serial: "Serial", software: "Software", profileType: "Profile type",
    full: "Full identity", legacyBasic: "Legacy basic profile", makeDefault: "Make default", remove: "Remove",
    addTitle: "Add device", addHelp: "Choose a reference FIT or a device from the Garmin FIT SDK catalog.", refTab: "Reference FIT", recommended: "Recommended", manualTab: "SDK catalog",
    ...
    manualTitle: "Choose a device from the SDK catalog", manualHelp: "Choose a Garmin model from the local catalog generated from the Garmin FIT SDK.",
    model: "Garmin model",
    fullHelp: "The profile uses full Garmin identity. Serial number and firmware must belong to the selected Garmin device.",
    legacyHelp: "This pre-release profile lacks a full Garmin identity and can no longer be used for patching. Delete it and recreate the device.",
    serialNumber: "Serial number", firmware: "Firmware version", saveManual: "Save device profile", catalog: "Garmin catalog",
    patchTitle: "Patch FIT file", sourceFile: "FIT file to patch", targetCheck: "Target device in patched FIT", patch: "Patch FIT", download: "Download patched FIT",
    changed: "Changed creator fields", verified: "Other FIT data verified unchanged.", ready: "Ready",
    importing: "Importing reference FIT…", imported: "Device profile imported.", saving: "Saving device profile…", saved: "Device profile saved.",
...
class GarminFitDeviceChangerCard extends HTMLElement {
  constructor() {
    ...
    this._profiles = []; this._catalog = []; this._catalogMeta = null; this._selected = null; this._downloadUrl = null; this._result = null; this._status = null;
    this._refFile = null; this._sourceFile = null; this._refLabel = "";
    this._manual = { device: "", serial: "", firmware: "", label: "" };
  ...
  _apply(data) {
    ...
    if (data?.catalog_meta && typeof data.catalog_meta === "object") this._catalogMeta = data.catalog_meta;
    if (!this._manual.device || !this._catalog.some((d) => d.key === this._manual.device)) {
      this._manual.device = this._catalog.find((d) => Number(d.product) === 4375)?.key || this._catalog[0]?.key || "";
    }
  ...
  async _saveManual() {
    ...
    try {
      const body = { device_key: this._manual.device, identity_mode: "full", label: this._manual.label, serial_number: this._manual.serial, software_version: this._manual.firmware };
  ...
  _wire() {
    ...
    q("#manual-device")?.addEventListener("change", (e) => { this._manual.device = e.target.value || ""; this._manual.serial = ""; this._manual.firmware = ""; this._render(); });
  ...
  _render() {
    ...
    const t = this._t(), selected = this._pickProfile(), manualDevice = this._catalog.find((d) => d.key === this._manual.device), selectedSupported = !!selected?.is_supported;
    const profiles = this._profiles.map((p) => `<option value="${esc(p.profile_id)}" ${p.profile_id === this._selected ? "selected" : ""}>${esc(p.label)}${p.is_default ? " (standard)" : ""}${p.is_supported === false ? ` — ${t.legacyBasic}` : ""}</option>`).join("");
    ...
    const catalogVersion = String(this._catalogMeta?.profile_version || "").replace(/Release$/i, "");
    const catalogLine = this._catalogMeta ? `${t.catalog}: FIT SDK ${esc(catalogVersion)} • ${esc(this._catalogMeta.generated_at || "")}` : "";
    ...
    this.shadowRoot.innerHTML = `<style>
      ...
      .target-check{display:flex;align-items:center;gap:11px;margin-top:11px;padding:10px 12px;border:1px solid var(--divider-color);border-radius:9px;background:var(--secondary-background-color)}
      .target-check ha-icon{--mdc-icon-size:21px;color:var(--primary-color);flex:0 0 auto}
      .target-check-copy{display:flex;flex-direction:column;gap:2px;min-width:0}
      .target-check-label{font-size:.76rem;color:var(--secondary-text-color)}
      .target-check strong{font-size:.94rem;overflow-wrap:anywhere}
      .target-check-meta{font-size:.78rem;color:var(--secondary-text-color)}
      ...
      .note.legacy{border-left:4px solid var(--warning-color,#ff9800)}
      ...
      .version{display:flex;gap:8px 14px;flex-wrap:wrap;justify-content:space-between;margin:10px 2px 0;font-size:.76rem}
      @media(max-width:600px){ha-card{padding:14px}.brand{padding:10px 12px}.brand-title{font-size:1.08rem}.grid{grid-template-columns:1fr}.section-body{padding-left:20px}.summary-meta{max-width:40%}.method-switch{grid-template-columns:1fr}.version{justify-content:flex-start}}
    </style><ha-card>
      ...
      <details id="details-profiles" class="section" ${this._open.profiles ? "open" : ""}><summary>${chevron}${icon("mdi:devices")}<span class="summary-title">${t.profiles}</span>${selected ? `<span class="summary-meta">${esc(selected.label)}</span>` : ""}</summary><div class="section-body">${this._profiles.length ? `<label>${t.target}<select id="profile">${profiles}</select></label>${selected ? `<div class="profile-meta meta"><span>${t.product}: ${selected.product}</span><span>${t.serial}: ${esc(selected.serial_number)}</span><span>${t.software}: ${esc(selected.software_version)}</span><span>${t.profileType}: ${selectedSupported ? t.full : t.legacyBasic}</span></div>${selected && !selectedSupported ? `<div class="note legacy">${t.legacyHelp}</div>` : ""}` : ""}<div class="actions"><button id="default" class="secondary" ${this._busy || selected?.is_default || !selectedSupported ? "disabled" : ""}>${t.makeDefault}</button><button id="delete" class="danger" ${this._busy ? "disabled" : ""}>${t.remove}</button></div>` : `<div class="help">${t.noProfiles}</div>`}</div></details>
      <details id="details-add" class="section" ${this._open.add ? "open" : ""}><summary>${chevron}${icon("mdi:plus-circle-outline")}<span class="summary-title">${t.addTitle}</span></summary><div class="section-body"><div class="help">${t.addHelp}</div><div class="method-switch"><button id="add-reference" class="method-tab ${this._addMode === "reference" ? "active" : ""}" type="button">${icon("mdi:file-check-outline")}<span>${t.refTab}<span class="recommended">${t.recommended}</span></span></button><button id="add-manual" class="method-tab ${this._addMode === "manual" ? "active" : ""}" type="button">${icon("mdi:database-outline")}<span>${t.manualTab}</span></button></div>${this._addMode === "reference" ? `<div class="method-heading">${t.refTitle}</div><div class="help">${t.refHelp}</div><div class="grid">${this._filePicker("ref-file",t.refFile,this._refFile)}<label>${t.optionalName}<input id="ref-label" type="text" value="${esc(this._refLabel)}"></label></div><div class="actions"><button id="import" class="primary" ${this._busy ? "disabled" : ""}>${t.import}</button></div>` : `<div class="method-heading">${t.manualTitle}</div><div class="help">${t.manualHelp}</div>${this._catalog.length ? `<div class="grid"><label class="full">${t.model}<select id="manual-device">${devices}</select></label><label>${t.serialNumber}<input id="manual-serial" type="text" inputmode="numeric" value="${esc(this._manual.serial)}"></label><label>${t.firmware}<input id="manual-firmware" type="text" inputmode="decimal" value="${esc(this._manual.firmware)}"></label><label class="full">${t.optionalName}<input id="manual-label" type="text" value="${esc(this._manual.label)}" placeholder="${esc(manualDevice?.label || "")}"></label></div><div class="note full">${t.fullHelp}</div><div class="actions"><button id="manual-save" class="primary" ${this._busy ? "disabled" : ""}>${t.saveManual}</button></div>` : `<div class="help">${t.error}: ${t.model}</div>`}`}</div></details>
      <details id="details-patch" class="section" ${this._open.patch ? "open" : ""}><summary>${chevron}${icon("mdi:file-edit-outline")}<span class="summary-title">${t.patchTitle}</span>${this._sourceFile ? `<span class="summary-meta">${esc(this._sourceFile.name)}</span>` : ""}</summary><div class="section-body">${this._filePicker("src-file",t.sourceFile,this._sourceFile)}${selected ? `<div class="target-check"><ha-icon icon="mdi:target"></ha-icon><div class="target-check-copy"><span class="target-check-label">${t.targetCheck}</span><strong>${esc(selected.label)}</strong><span class="target-check-meta">${t.product}: ${selected.product} • ${selectedSupported ? t.full : t.legacyBasic}</span></div></div>` : ""}<div class="actions"><button id="patch" class="primary" ${this._busy || !this._profiles.length || !selectedSupported ? "disabled" : ""}>${t.patch}</button></div>${result}</div></details>${status}<div class="meta version"><span>v${CARD_VERSION}</span>${catalogLine ? `<span>${catalogLine}</span>` : ""}</div>
    </ha-card>`;
    this._wire();
  }
}
